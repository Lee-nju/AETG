...
//约束的四种类型 但最后一种可以利用逆否命题c转换成第一种情况
int to_option(char (*op)[3]){
    int option = 0;
    int k = sizeof(op) / sizeof(op[0]);
    for (int i = 0; i < k; ++i) {
        if (!strcmp(op[i], "=")) {
            option = option << 1 ;
        }else if(!strcmp(op[i], "!=")){
...
void to_constriant(int *pi,int *vi,int option,vector<vector<bool>> &v,const vector<int> &vp){
    --vi[0]; //把参数变成参数号
    --vi[1];
    for (int i = 0; i < nrow; ++i) {
        int p_first = -1,p_second = -1; //代表找到参数的行号和列号
        for (int j = 0; j < t; ++j) { //遍历p[i][] 看看是不是约束行 记录每个出现的约束参数
            if(p[i][j] == pi[0] ){
                p_first = j;
            }
            if (p[i][j] == pi[1] ) {
        ...
        unsigned long ncol = v[i].size();
        if (p_first >= 0 && p_second >= 0) { //说明这一行是约束行
            for (int k = 0; k < ncol; ++k) { //给第i行加约束
                int value[t]; //把i行的列号转换成对应参数的值
                int m = k; //m用于保存k
                for (int c = t - 1; c >= 0; --c) {
                    int pj = p[i][c]; //pj保存参数号
                    value[c] = m % vp[pj];
                    m /= vp[pj];
                }
                if (option == 0) {
                    bool pass = vi[0] == value[p_first] && vi[1] == value[p_second];
                    if (!pass) { //如果不满足约束 将其置为false
                        v[i][k] = false;
                    }
                } else if(option == 1){
                    bool pass = vi[0] == value[p_first] && vi[1] != value[p_second];
...
void to_constriant2(int *pi,int *vi,int option,vector<vector<bool>> &v,const vector<int> &vp){
    --vi[0]; //把参数变成参数号
    --vi[1];
    --vi[2];
    for (int i = 0; i < nrow; ++i) {
        int p_first = -1,p_second = -1,p_third = -1; //代表找到参数的行号和列号
        for (int j = 0; j < t; ++j) { //遍历p[i][] 看看是不是约束行 记录每个出现的约束参数
            if(p[i][j] == pi[0] ){
                p_first = j;
            }
        ...
        unsigned long ncol = v[i].size();
        if (p_first >= 0 && p_second >= 0 && p_third >= 0) { //说明这一行是约束行
            for (int k = 0; k < ncol; ++k) { //给第i行加约束
                int value[t]; //把i行的列号转换成对应参数的值
                int m = k; //m用于保存k
                for (int c = t - 1; c >= 0; --c) {
                ...
                if (option == 0) {
                    bool pass = (vi[0] == value[p_first] && vi[1] == value[p_second]) && vi[3] == value[p_third];
                    if (!pass) { //如果不满足约束 将其置为false
                        v[i][k] = false;
                    }
                } else if(option == 1){
...
int main(int argc, const char * argv[]) {
    //重定向输入流
    freopen("/Users/ligang/code_study/test1.txt", "r", stdin);
    freopen("/Users/ligang/code_study/testcase2.txt", "w", stdout);
    
    num = 0; //参数个数初始化为0
    vector<int> vp; //参数个数取值列表
    string str; //保存文件的每一行
    while(cin.peek() != EOF){ //文件不为空时遍历
        getline(cin, str); //获得第一行 理应为[parameter] 或者 [constraint]
        ...
        else if(str.find("[constraint]") != str.npos){ //进入约束处理--------------------
            while (cin.peek() == 'p') { //第一个字符为‘p’ 说明这是一个约束行
                char ch[40]; //用字符数组存储一行的内容
                cin >> ch; //把这一行内容存入str里面
                int constrian_num = 0; //记录此约束是几个参数之间的约束
                unsigned long k = strlen(ch);
                for (int i = 0; i < k; ++i) {
                    if (ch[i] == 'p') {
                        ++constrian_num; //每次找到一个p加1
                    }
                }
                if (constrian_num == 2) { //两个参数之间的约束时
                    int pi[2],vi[2]; //分别为参数1、2和值1、2
                    char op[2][3]; //存储操作符
                    //给参数和操作符赋值
                    sscanf(ch, "p%d%[=|!=]%d=>p%d%[=|!=]%d", &pi[0],op[0],&vi[0],&pi[1],op[1],&vi[1]);
                    int option = to_option(op);
                    
                    //把不满足约束的uncombs置为false
                    to_constriant(pi, vi, option, uncover_target_combs, vp);
                }
                else if(constrian_num == 3){ //三个参数之间的约束时
                    int pi[3],vi[3]; //分别为参数1、2、3和值1、2、3
                    char op[3][3]; //存储操作符
                    //给参数和操作符赋值
                    sscanf(ch, "p%d%[=|!=]%d%*[&&]p%d%[=|!=]%d=>p%d%[=|!=]%d", &pi[0],op[0],&vi[0],&pi[1],op[1],&vi[1],&pi[2],op[2],&vi[2]);
                    int option = to_option(op);
                    to_constriant2(pi, vi, option, uncover_target_combs, vp);
                }
                cin.ignore();
            }
        }
